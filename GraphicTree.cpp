...
void QtBtree::InsDel()
{
    ...
    if (dlg.ui.checkBox_insert->isChecked())
    {
    ...
    if (dlg.ui.checkBox_delete->isChecked())
    {
        int key = dlg.ui.lineEdit_keys->text().toInt();
        if (bt->Search(key).tag == false)//no existe
        {
    ...
    if (dlg.ui.checkBox_Search->isChecked())
    {
        int key = dlg.ui.lineEdit_keys->text().toInt();

        if (bt->Search(key).tag == true)
        {
            QMessageBox::warning(this, "Exitos!", "El nodo se encontro!");
            return;
        }

        else
        {
            QMessageBox::warning(this, "Error!", "El nodo no se ha encontrado, intente nuevamente");
            return;
        }
    }
}
...
{
    ui.setupUi(this);
    ui.m_lineEdit->setValidator(new QRegExpValidator(QRegExp("[1-9]")));
...
{
    int xx = this->pt.x();
    for (unsigned int i = 0; i < rowNodes.size(); i++)
    {
...
bool DrawAll::isNeedNewRow()
{
    ...
    int LastChildSum=0;
    unsigned long LastRowNodeNum = rows[rows.size() - 2].rowNodes.size();
    for (unsigned long i = 0; i < LastRowNodeNum; i++)
    {
...
void buscar()
{
   ...
   /*InsAndDelDLG dlg;
    if(search(dlg.ui.lineEdit_keys->text().toInt())==0)
        QMessageBox::information(this,"Search","No se encontro el nodo a buscar");
    else
        QMessageBox::information(this,"Search","Se encontro el nodo a buscar");*/
}
